...
function draughtSocketRoute() {
  const wss = new WebSocket.Server({ noServer: true });
  let games = {}; // Store games for draughts
  ...
  wss.on('connection', function connection(ws) {
    ws.on("message", (message) => {
      ...
      switch (data.type) {
        ...
        case "move":
          const gameRoom = Object.values(games).find((room) => room.players.includes(ws)
          );
          if (gameRoom) {
        ...
        case "exitGame":
          const exitGameRoom = Object.values(games).find((room) => room.players.includes(ws)
          );
          if (exitGameRoom) {
    ...
    ws.on("close", () => {
      const disconnectedGame = Object.values(games).find((room) => room.players.includes(ws)
      );
      if (disconnectedGame) {
...
exports.draughtSocketRoute = draughtSocketRoute;
